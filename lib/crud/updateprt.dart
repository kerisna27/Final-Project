...
class UpdateQuestion extends StatefulWidget {
  final Question pertanyaan;
  UpdateQuestion({this.pertanyaan});
  @override
...
class _UpdateQuestionState extends State<UpdateQuestion> {
  ...
  void addTambahQuestion() async {
    var url = 'http://192.168.43.76/db_petshop/update_pertanyaan.php';
  ...
  @override
  void initState() {
    _controllerNotelepon.text = widget.pertanyaan.notelepon;
    _controllerNama.text = widget.pertanyaan.nama;
    _controllerAlamat.text = widget.pertanyaan.alamat;
    _controllerPertanyaan.text = widget.pertanyaan.pertanyaan;
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Update Data Pertanyaan'),
      ),
      body: ListView(
